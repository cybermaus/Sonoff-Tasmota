# Cybermaus fork
Just some minor personal changes to default Theo Ahrends build
I may occasionally merge, when I need it myself.
- KNX improvement: Enhanced UDP sends 5 times with small delay rather then 3 times immediate (try to end up in different aggregation WiFi packets)
- KNX improvement: when Power command received via KNX, then do not send via KNX (no reflections)
- RTC improvement: keep time during software reboot by storing in RTC memory (does not work for unplug reboots)
- RTC improvement: Reject suspect NTP update when drift > 100 hours, unless two NTP packets agree on such drift.
